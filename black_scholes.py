...
def call_value(S, K, T, r, sigma):
    ...
    return float(S * _norm_cdf(_d1(S, K, T, r, sigma)) - K * exp(-r * T) * _norm_cdf(
        _d2(S, K, T, r, sigma)
    ))
...
def put_value(S, K, T, r, sigma):
    ...
    return float(exp(-r * T) * K * _norm_cdf(-_d2(S, K, T, r, sigma)) - S * _norm_cdf(
        -_d1(S, K, T, r, sigma)
    ))
...
def call_delta(S, K, T, r, sigma):
    ...
    return float(_norm_cdf(_d1(S, K, T, r, sigma)))
...
def call_vega(S, K, T, r, sigma):
    ...
    return float(S * _norm_pdf(_d1(S, K, T, r, sigma)) * sqrt(T))
